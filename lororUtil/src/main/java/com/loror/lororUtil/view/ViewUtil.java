...
public class ViewUtil {
	...
	private static void injectObject(Object handler, ViewFinder finder) {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (find != null) {
				try {
					View annotations = finder
							.findViewById(find.value() == -1 ? getResourceId(name, globalIdClass) : find.value());
					if (annotations != null) {
	...
	private static int getResourceId(String variableName, Class<?> idClass) {
		try {
			Field idField = idClass
					.getDeclaredField(suiteHump ? TextUtil.humpTo_lowercase(variableName) : variableName);
